...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if($inputToCart['size']==""){
            return back()->with('message','Por favor seleccione medida');
        }else{
            ...
            if($stockAvailable->stock>=$inputToCart['quantity']){
                ...
                if($count_duplicateItems>0){
                    return back()->with('message','Este articulo ya se agrego');
                }else{
                    Cart_model::create($inputToCart);
                    return back()->with('message','Agregado al carrito');
                }
            }else{
                return back()->with('message','Stock no esta disponible!');
            }
        }
    }
    public function deleteItem($id=null){
        ...
        Session::forget('coupon_code');
        $delete_item->delete();
        return back()->with('message','Exito eliminado!');
    }
