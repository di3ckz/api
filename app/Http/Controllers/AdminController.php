...
class AdminController extends Controller
{
    ...
    public function updatAdminPwd(Request $request){
        ...
        if(Hash::check($current_password,$check_password->password)){
            ...
            return redirect('/admin/settings')->with('message','Actualización de password con exito');
        }else{
            return redirect('/admin/settings')->with('message','Password actual incorrecta');
        }
    }
    ...
    /*public function login(Request $request){
        if($request->isMethod('post')){
            ...
            if(Auth::attempt(['email'=>$data['email'],'password'=>$data['password'],'admin'=>'1'])){
                ...
                return redirect('admin')->with('message','La cuenta es incorrecta!');
            }
        }else{
